...
$('document').ready(function() {
	...
	$('.table .btn-primary').on('click',function(event){		
		...
		$.get(href, function(vehicleMovement, status){
			...
			$('#ddlLocation1Edit').val(vehicleMovement.locationStartid);
			$('#ddlLocation2Edit').val(vehicleMovement.locationEndid);
			
			var date1 = vehicleMovement.dateStart.substr(0,10);
			var date2 = vehicleMovement.dateEnd.substr(0,10);
			$('#date1Edit').val(date1);
			$('#date2Edit').val(date2);
	...
	$('.table #detailsButton').on('click',function(event) {
		...
		$.get(href, function(vehicleMovement, status){
			...
			$('#ddlLocation1Details').val(vehicleMovement.locationStartid);
			$('#ddlLocation2Details').val(vehicleMovement.locationEndid);
